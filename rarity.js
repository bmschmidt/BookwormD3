...
function timeSpark(query) {
    ...
    a.enter().append("svg").attr("width",80).attr("height",20).attr("class","sparkline").style("position","absolute")
...
function setSparkWatcher() {
    ...
    d3.select("#svg").selectAll("text").on("mouseover",function(d) {
        var quer = {"database": "movies","method":"return_json",
                    "plotType": "sparkline",
                    "search_limits": {"MovieYear":{"$lte":2013},
                        "word": [d.unigram]
                    },
                    "aesthetic": {
                        "x": "MovieYear",
	...
	svg.style("top",d3.event.pageY + "px")
