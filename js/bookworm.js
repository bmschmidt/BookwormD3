...
BookwormClasses = {
    ...
    updatePlot : function(callback) {
        ...
        if (bookworm.silent===undefined) {
            // you can turn off window hashing by setting bookworm.silent to anything.
    ...
    mapPoints: function(proj,timeHandler) {
        ...
        var extent = d3.extent(bookworm.data.map(function(d) {return d[bookworm.query.aesthetic.size]}))

        var sizeScale = d3.scale.sqrt().range([minsize,topsize]).domain([0,extent[1]])
    ...
    addFilters : function(fields,attachTo,limitthing) {
        ...
        // Here are the different kinds of filters that can exist. There are scoped here
        // so they have access to the `bookworm` variable just defined
        var filters = {
            "textArray" : function() {
                var that = {}
                var box;
...
makeVega = function(worm) {
    var that;
    var bookworm = worm || bookworm;

    function varType(varname) {
	
	// Translates from Bookworm data types to Vega data types using descriptions pulled from the API.

	//Should use the 'dataType' field, not 'type', but that's currently not supported.
	
	varname  = varname.replace(/^\*/,"")

	var quantVariables = bookworm.variableOptions.quantitative.map(function(d) {return d.dbname})
	if (quantVariables.indexOf(varname) > -1) {return "quantitative"}
	console.log(quantVariables)
	var variableMatches = bookworm.variableOptions.options.filter(function(d) {return d.dbname==varname})
	
	if (variableMatches.length==0) {
	    console.log("No such variable as " + varname + " in the Bookworm: dying gracefully.")
	} else {
	    type = variableMatches[0].type
	    
	    if (type=="character") {return "ordinal"}
	    if (type.endsWith("_year") || type.endsWith("_day") || type.endsWith("_week")) {return "temporal"}
	    if (type=="integer") {return "quantitative"}
	}
    }


    // Encoding is the object we pass to vega-lite
    
    var encoding = {}

    d3.keys(bookworm.query.aesthetic).forEach(function(key) {
	var output = {}
	output["field"] = bookworm.query.aesthetic[key]
	output["type"] = varType(bookworm.query.aesthetic[key])
	encoding[key] = output;
    })
    
    var spec = {
	"data": {
	    "values": bookworm.data
	},
	"mark": bookworm.query.plotType,
	"encoding": encoding 
    }
...
// SHA hashing by Chris Veness. Used as an identifier for queries.
